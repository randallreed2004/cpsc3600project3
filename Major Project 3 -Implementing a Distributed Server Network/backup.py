...
##############################################################################################################
    ...
    def broadcast_message_to_servers(self, message, ignore_host_id=None):
        ...
        for server_id in self.hosts_db:
            if self.hosts_db[server_id] and server_id is not ignore_host_id and server_id is not self.id:
...
##############################################################################################################
    ...
    def handle_server_registration_message(self, io_device, message):
        ...
        if message.source_id in self.hosts_db:
            ...
            self.s.write_buffer += msg
            return
        
        # Message came from adjacent server
        if message.last_hop_id == 0:
            self.new_data = ServerConnectionData(message.source_id, message.server_name, message.server_info)
            self.new_data.first_link_id = message.source_id

            self.hosts_db[message.source_id] = self.new_data
            self.adjacent_server_ids.append(message.source_id)
            self.sel.modify(io_device.fileobj, 
                            selectors.EVENT_READ | selectors.EVENT_WRITE, 
                            self.new_data)
            
            msg = ServerRegistrationMessage.bytes(self.id, self.id, self.server_name, self.server_info)
            self.send_message_to_host(message.source_id, msg)
            for id, data in self.hosts_db.items():
                if isinstance(data, ServerConnectionData) and id is not message.source_id:
                    msg = ServerRegistrationMessage.bytes(data.id, self.id, data.server_name, data.server_info)
                    self.send_message_to_host(message.source_id, msg)

        # Not adjacent - first link ID is just the last hop of the message received
        else:
            self.new_data = ServerConnectionData(message.source_id, message.server_name, message.server_info)
            self.new_data.first_link_id = message.last_hop_id
            self.hosts_db[message.source_id] = self.new_data
            msg = ServerRegistrationMessage.bytes(message.source_id, self.id,
                                                  message.server_name, message.server_info)
            self.broadcast_message_to_servers(msg, self.new_data.id)
